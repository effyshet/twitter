...
contract Twitter {
    ...
    function registerAccount(string memory _name) external {
        ...
        User storage _recentUser = users[msg.sender];
    ...
    function postTweet(string calldata _content) external accountExists(msg.sender) {     
        Tweet memory _recentTweet = Tweet(
            nextTweetId,
            msg.sender,
            _content,
            block.timestamp
    ...
    function sendMessage(address _recipient, string calldata _content) external {
        Message memory _recentMessage;
        _recentMessage.messageId = nextMessageId;
        _recentMessage.content = _content;
        _recentMessage.from = msg.sender;
        _recentMessage.to = _recipient;
        users[msg.sender].conversations[_recipient].push(_recentMessage);
        users[_recipient].conversations[msg.sender].push(_recentMessage);
        nextMessageId++;
    }
